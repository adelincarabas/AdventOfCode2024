...
function solvePart1(input) {
    let aFirstRowNumbers = [];
    let aSecondRowNumbers = [];
    for(x of input){
        aFirstRowNumbers.push(parseInt(x.split("   ")[0]));
        aSecondRowNumbers.push(parseInt(x.split("   ")[1]));
    }
    aFirstRowNumbers.sort(function(a, b) {
        return a - b;
    });
    aSecondRowNumbers.sort(function(a, b) {
        return a - b;
    });
    let iTotalDistance = 0;
    for(let i = 0; i < aFirstRowNumbers.length; i++){
        iTotalDistance = iTotalDistance + Math.abs(aFirstRowNumbers[i] - aSecondRowNumbers[i]);
    }
...
function solvePart2(input) {
    ...
    let iSimilarityScore = 0;
    for(let x of aFirstRowNumbers){
        let iCount = 0;
        for(let y of aSecondRowNumbers){
            if(x == y){
                iCount ++;
            }
        }
        iSimilarityScore = iSimilarityScore + x * iCount;
    }
